...
package com.facedetection.facedetector
...
import com.facedetection.GraphicOverlay
...
*/
        ...
        //center of the box and the cheeks difference
        Log.e("Face Contour", "draw: Eular angleX ${face.headEulerAngleX}")
        Log.e("Face Contour", "draw: Eular angleY  ${face.headEulerAngleY}")
        Log.e("Face Contour", "draw: Center coordinates $x , $y")

        val leftX = translateX(face.allContours[14].points[0].x)
        val leftY = translateX(face.allContours[14].points[0].y)
        val rightX = translateX(face.allContours[13].points[0].x)
        val rightY = translateX(face.allContours[13].points[0].y)
        val leftDistance = sqrt((x - leftX) * (x - leftX) + (y - leftY) * (y - leftY))
        val rightDistance = sqrt(((x - rightX) * (x - rightX)) + ((y - rightY) * (y - rightY)))
        val distanceDifference = leftDistance - rightDistance
        var faceDirection: String = ""

        if (distanceDifference < 5 && distanceDifference > -5) {
            faceDirection = "face direction forward"
        } else if (distanceDifference < 20 && distanceDifference > -20)
            faceDirection = if (leftDistance > rightDistance)
                "face direction left"
            else
        ...
        canvas.drawText(
            "$faceDirection $distanceDifference", left, top + yLabelOffset,
            idPaints[colorID]
        )
        yLabelOffset += lineHeight


        Log.e("Face Contour", "draw: right cheek coordinates $rightX , $rightY}")
        Log.e("Face Contour", "draw: left cheek coordinates $leftX , $leftY")
        Log.e("Face Contour", "draw: distance from left cheek $leftDistance")
        Log.e("Face Contour", "draw: distance from right cheek $rightDistance")
        Log.e("Face Contour", "draw: distance difference is $distanceDifference")
        Log.e("Face Contour", "draw: Where is face?: $faceDirection")


        //Cheeks points
        /*canvas.drawCircle(
                translateX(face.allContours[13].points[0].x),
                translateY(face.allContours[13].points[0].y),
                FACE_POSITION_RADIUS,
                facePositionPaint
        )

        canvas.drawCircle(
                translateX(face.allContours[14].points[0].x),
                translateY(face.allContours[14].points[0].y),
                FACE_POSITION_RADIUS,
                facePositionPaint
        )*/
